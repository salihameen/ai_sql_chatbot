...
# NEW: summarizer model
summarizer = pipeline("text2text-generation", model="google/flan-t5-small")

def question_to_sql(question):
    prompt = f"Convert this question to an SQL query for a table named employees: {question}"
    result = generator(prompt, max_length=100)
    sql_query = result[0]['generated_text']
    return sql_query

def summarize_result(question, df):
    prompt = f"Summarize the answer to this question: '{question}' based on this data: {df.head(5).to_dict()}"
    summary = summarizer(prompt, max_length=80)
